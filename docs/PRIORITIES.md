...
3. **~~Whose account is `d0fa6b3d72170539438800a907ec5323`?~~ ANSWERED 2026-09-22: `swarm-work-emailer`.** `npx wrangler whoami`, signed in as `echo@swarm.work`, lists three accounts and names that id `swarm-work-emailer`. Section 1 of this file was right; `docs/DEPLOYMENT.md` and `docs/HANDOVER.md` were wrong and have been corrected. This also settles two things: nothing has to be migrated elsewhere before a remote database is created, and the premise that "D1 and R2 are now on it" was false - `npx wrangler d1 list` returns nothing, so no remote database has ever existed. 3.2 is unblocked.
