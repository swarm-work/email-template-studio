...
| 6   | Sign-in "succeeds", nothing happens                                                                                                                                                    | `PasswordGate` probed the API once on mount and never again; the SDK gives no redirect of its own                                                                                                                                                                                                                                                                                                                              | **our code**                    | `callbacks.onEvent` → gate re-probes, leaves `/authenticate`, and if the API still refuses, shows its reason under the form                                                                                                             | code done; **untested**: `PasswordGate.test.tsx:12` stubs `StytchSignIn` with a component that never calls `onSignedIn`, and the path has not been seen in a browser |
