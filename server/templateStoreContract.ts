...
export function describeTemplateStore(name: string, makeStore: () => TemplateStore): void {
  describe(`TemplateStore contract: ${name}`, () => {
    it('creates a template at version 1 and revision 1', async () => {
      ...
      expect(await store.get('tpl_welcome')).toEqual(created)
    })
    ...
    it('appends versions 2 and 3 and moves the revision forward with them', async () => {
      ...
      const v2 = expectSaved(
        await store.addVersion('tpl_welcome', v1.revision, codeVersion({ note: 'second' }), LATER_CTX),
      )
      ...
      const v3 = expectSaved(
        await store.addVersion('tpl_welcome', v2.revision, codeVersion({ note: 'third' }), LATER_CTX),
      )
    ...
    it('refuses a stale expectedRevision, writes nothing, and hands back the current template', async () => {
      ...
      const outcome = await store.addVersion('tpl_welcome', 1, codeVersion({ note: 'stale' }), LATER_CTX)
    ...
    it('answers not-found for a template that does not exist', async () => {
      const store = makeStore()
      expect(await store.get('tpl_missing')).toBeNull()
      expect(await store.listVersions('tpl_missing')).toBeNull()
      expect(await store.remove('tpl_missing')).toBe('not-found')
      expect((await store.addVersion('tpl_missing', 1, codeVersion(), CTX)).status).toBe('not-found')
      expect((await store.updateMetadata('tpl_missing', 1, { name: 'x' }, CTX)).status).toBe('not-found')
    })

    it('removes a template and its versions with it', async () => {
      const store = makeStore()
      const created = expectSaved(await store.create(newTemplate('welcome'), CTX))
      await store.addVersion('tpl_welcome', created.revision, codeVersion(), CTX)

      expect(await store.remove('tpl_welcome')).toBe('deleted')
      expect(await store.get('tpl_welcome')).toBeNull()
      // The cascade: history must not outlive the template it belongs to.
      expect(await store.listVersions('tpl_welcome')).toBeNull()
    ...
    it('lists versions newest first', async () => {
      ...
      await store.addVersion('tpl_welcome', v2.revision, visualVersion({ note: 'third' }), LATER_CTX)

      const versions = await store.listVersions('tpl_welcome')
    ...
    it('lists templates newest-updated first', async () => {
      ...
      expect((await store.list()).map((template) => template.slug)).toEqual(['second', 'first'])
    })
    ...
    it('bumps the revision on a metadata change without adding a version', async () => {
      ...
      expect(await store.listVersions('tpl_welcome')).toHaveLength(1)
    })

    it('refuses a stale expectedRevision on a metadata change too', async () => {
      const store = makeStore()
      const created = expectSaved(await store.create(newTemplate('welcome'), CTX))
      await store.updateMetadata('tpl_welcome', created.revision, { name: 'Renamed' }, CTX)

      const outcome = await store.updateMetadata('tpl_welcome', 1, { name: 'Also renamed' }, CTX)
      expect(outcome.status).toBe('conflict')
      expect((await store.get('tpl_welcome'))?.name).toBe('Renamed')
    })
    ...
    it('refuses a slug that another template already holds', async () => {
      ...
      expect(
        (await store.updateMetadata('tpl_reset', second.revision, { slug: 'welcome' }, CTX)).status,
      ).toBe('slug-taken')
      // Re-sending its OWN slug is not a clash, it is a no-op rename.
      expect((await store.updateMetadata('tpl_reset', second.revision, { slug: 'reset' }, CTX)).status).toBe(
        'saved',
      )
    })
    ...
    it('refuses a create over an id that already exists, leaving the history intact', async () => {
      const store = makeStore()
      const created = expectSaved(await store.create(newTemplate('welcome'), CTX))
      await store.addVersion('tpl_welcome', created.revision, codeVersion({ note: 'second' }), LATER_CTX)
      ...
      const survivor = await store.get('tpl_welcome')
    ...
    it('converting a visual template to code is one way: the kind follows the current version', async () => {
      ...
      const converted = expectSaved(
        await store.addVersion('tpl_newsletter', created.revision, codeVersion({ note: 'converted' }), CTX),
      )
      ...
      expect((await store.list())[0].kind).toBe('code')
      // The visual version is still in the history; conversion adds, never deletes.
      expect((await store.listVersions('tpl_newsletter'))?.map((v) => v.kind)).toEqual(['code', 'visual'])
    })
  })
}
